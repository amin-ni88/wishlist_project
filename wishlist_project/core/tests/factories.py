...
class WishListItemFactory(factory.django.DjangoModelFactory):
    ...
    price = factory.LazyFunction(
        lambda: fake.pydecimal(left_digits=3, right_digits=2))
...
class SubscriptionFactory(factory.django.DjangoModelFactory):
    ...
    end_date = factory.LazyFunction(
        lambda: fake.date_this_year(after_today=True))
    is_active = True
...
class ContributionFactory(factory.django.DjangoModelFactory):
    ...
    amount = factory.LazyFunction(
        lambda: fake.pydecimal(left_digits=2, right_digits=2))
    created_at = factory.LazyFunction(lambda: fake.date_time_this_year())
...
class NotificationFactory(factory.django.DjangoModelFactory):
    ...
    notification_type = factory.Iterator(
        ['CONTRIBUTION', 'WISHLIST_SHARED', 'ITEM_PRICE_DROP'])
