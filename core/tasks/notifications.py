...
def send_email_notification(user_id, subject, template_name, context):
    ...
    try:
        ...
        html_message = render_to_string(
            f'notifications/{template_name}.html', context)
...
def notify_wishlist_invite(invitation_id):
    ...
    try:
        ...
        invitation = ShareInvitation.objects.select_related(
            'wishlist', 'invited_by').get(id=invitation_id)
...
def notify_item_price_change(item_id, old_price, new_price):
    ...
    try:
        item = WishListItem.objects.select_related(
            'wishlist__owner').get(id=item_id)
